...
SELECT 
    HOUR(oder_time) AS hour, COUNT(order_id) AS order_count
FROM
    order_type
...
FROM
    ...
    FROM
        order_type
    JOIN order_details ON order_type.order_id = order_details.order_id
    GROUP BY order_type.order_date) AS order_quantity;
...
FROM (
    ...
    JOIN 
        order_type ot ON ot.order_id = od.order_id 
    GROUP BY 
        ot.order_date
) AS sales;
